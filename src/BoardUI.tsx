...
import { PromotionEntities } from './PromotionEntities';
// import App from '../App';

function BoardUI({game}: {game: Game}) {

  const board = game.getBoard();
  const grid: (Entity|null)[][] = board.getGrid();
  const [moves, setMoves] = useState<boolean[][]>(Array(8).fill(null).map(() => Array(8).fill(false)));
  const [currentEntity, setCurrentEntity] = useState<Entity | null>(null);
  const [isWhiteTurn, setIsWhiteTurn] = useState<boolean>(true);
  const [canPromotePawn, setCanPromotePawn] = useState<boolean>(false);
